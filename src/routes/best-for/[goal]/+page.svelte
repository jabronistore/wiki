<script lang="ts">
	...
	const canonicalUrl = $derived(`${SITE_URL}/best-for/${goal.slug}`);
	const title = $derived(`Best Compounds for ${goal.title} (2026) | Peptide Database`);
	const description = $derived(
		`${ranked.length} compounds ranked for ${goal.title.toLowerCase()}. ${goal.description} Evidence-based ranking from research data.`
	);
	...
	const jsonld = $derived({
		...
		'@graph': [
			{
				'@type': 'Article',
				headline: `Best Compounds for ${goal.title}`,
			...
			},
			{
				...
				itemListElement: [
					...
					{ '@type': 'ListItem', position: 2, name: 'Best For', item: `${SITE_URL}/best-for` },
	...
	function getItemHref(id: string): string {
		const item = getItemBySlug(id);
		if (!item) return `/peptides/${id}`;
		return `${getItemUrlPrefix(item)}/${id}`;
	}
</script>
...
<SEO
	{title}
	{description}
	keywords="best compounds for {goal.title.toLowerCase()}, {goal.title.toLowerCase()} peptides, compound ranking"
...
<div class="bf-page">
	...
	<nav aria-label="Breadcrumb" class="bf-breadcrumb">
		<ol>
			...
			<li><a href="/best-for">Best For</a><ChevronRight class="sep h-3.5 w-3.5" /></li>
			<li><span class="current">Best for {goal.title}</span></li>
		</ol>
	</nav>

	<header class="bf-header">
		<h1>Best Compounds for {goal.title}</h1>
		<p class="bf-desc">{goal.description}</p>
		<p class="bf-count">{ranked.length} compounds ranked by research effectiveness</p>
	</header>

	<!-- Ranking list -->
	<div class="bf-list">
		{#each ranked as peptide, i}
			<a href={getItemHref(peptide.id)} class="bf-card">
