<script lang="ts">
	...
	interface Props {
		peptide: Peptide;
		allPeptides: PeptideSummary[];
		urlPrefix?: string;
		browseLabel?: string;
	}
	let { peptide, allPeptides, urlPrefix = '/peptides', browseLabel = 'Peptides' }: Props = $props();
	...
	const canonicalUrl = $derived(`${SITE_URL}${urlPrefix}/${peptide.id}`);
		...
		'@graph': [
			...
			},
			{
				...
				itemListElement: [
					...
					{ '@type': 'ListItem', position: 2, name: browseLabel, item: `${SITE_URL}${urlPrefix}` },
		...
		<div class="peptide-content">
			...
			<nav aria-label="Breadcrumb" class="mb-8">
				<ol class="flex items-center gap-1.5 text-sm text-muted-foreground">
					...
					<li class="flex items-center gap-1.5">
						<a href={urlPrefix} class="transition-colors hover:text-foreground">{browseLabel}</a>
						<ChevronRight class="h-3.5 w-3.5" />
					</li>
					<li>
			...
			<!-- Community Section -->
			<PeptideCommunitySection peptideId={peptide.id} peptideName={peptide.name} {urlPrefix} />
					...
					{#if related.length > 0}
						<section class="sect scroll-mt-8">
							...
							<div class="related-grid">
								{#each related as rel}
									{@const relSummary = allPeptides.find((p) => p.id === rel.id)}
									<a
										href="{relSummary ? getItemUrlPrefix(relSummary) : urlPrefix}/{rel.id}"
										class="related-card"
									>
